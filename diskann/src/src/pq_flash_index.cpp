...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
    ...
    // // ── Dump Frequency cache list + neighbor connections ──────────────────
    // {
    //     std::set<uint32_t> cached_set(node_list.begin(), node_list.end());
    //     for (auto node : node_list)
    //         diskann::cout << "[FREQ_NODE] " << node << "\n";
    //     std::vector<T *> coord_bufs(node_list.size(), nullptr);
    //     std::vector<std::pair<uint32_t,uint32_t*>> nbr_bufs;
    //     for (size_t i = 0; i < node_list.size(); i++)
    //         nbr_bufs.emplace_back(0, new uint32_t[_max_degree + 1]);
    //     auto rs = read_nodes(node_list, coord_bufs, nbr_bufs);
    //     for (uint32_t i = 0; i < rs.size(); i++) {
    //         if (!rs[i]) continue;
    //         uint32_t src = node_list[i];
    //         for (uint32_t j = 0; j < nbr_bufs[i].first; j++) {
    //             uint32_t dst = nbr_bufs[i].second[j];
    //             diskann::cout << "[FREQ_EDGE] " << src << " " << dst
    //                           << " " << (cached_set.count(dst) ? "cached" : "cold") << "\n";
    //         }
    //         delete[] nbr_bufs[i].second;
    //     }
    // }
    // // ─────────────────────────────────────────────────────────────────────
                                               ...
                                               const bool shuffle)
{
    ...
    // // ── Special node hop tracking ─────────────────────────────────────────
    // std::unordered_map<uint32_t, uint32_t> node_hop;
    // // Mark medoids as hop 0
    // for (uint64_t miter = 0; miter < _num_medoids; miter++)
    //     node_hop[_medoids[miter]] = 0;
    // diskann::cout << "[BFS_CACHE] medoid=" << _medoids[0] << " hop=0" << std::endl;
    // // ─────────────────────────────────────────────────────────────────────
    ...
    // // ── DUMP: BFS cache list + neighbor connections ───────────────────
    // {
    //     std::set<uint32_t> cached_set(node_list.begin(), node_list.end());
    //     // Print cached nodes
    //     for (auto node : node_list)
    //         diskann::cout << "[BFS_NODE] " << node << "\n";
    //     // Read neighbors of all cached nodes
    //     std::vector<T *> coord_bufs(node_list.size(), nullptr);
    //     std::vector<std::pair<uint32_t,uint32_t*>> nbr_bufs;
    //     for (size_t i = 0; i < node_list.size(); i++)
    //         nbr_bufs.emplace_back(0, new uint32_t[_max_degree + 1]);
    ...
    // // ── Print ALL cached nodes with hop level ─────────────────────────────
    // diskann::cout << "\n[BFS_CACHE] Full cache report (k=" 
    //             << num_nodes_to_cache << "):" << std::endl;
    // diskann::cout << "[BFS_CACHE] medoid=" << _medoids[0] << std::endl;

    // std::set<uint32_t> cached_set(node_list.begin(), node_list.end());

    // // Print node_hop entries that are in node_list
    // for (auto& [node, hop] : node_hop)
    // {
    //     if (cached_set.count(node))
    //     {
    //         diskann::cout << "[BFS_CACHE] node=" << node
    //                     << " hop=" << hop
    //                     << " cached=YES"
    //                     << std::endl;
    //     }
    // }
    // // Print nodes NOT cached but tracked in node_hop
    // for (auto& [node, hop] : node_hop)
    // {
    //     if (!cached_set.count(node))
    //     {
    ...
    //     }
    // }
    diskann::cout << "[BFS_CACHE] done" << std::endl;
    // ─────────────────────────────────────────────────────────────────────
}
                                                 ...
                                                 QueryStats *stats)
{
    ...
    // VectorTailCache: workload logging
    uint64_t _wl_query_id = 0;
    if (_workload_logger.is_enabled()) {
        static std::atomic<uint64_t> _global_qid{0};
        _wl_query_id = _global_qid.fetch_add(1);
        _workload_logger.begin_query(_wl_query_id);
    }
    ...
    while (retset.has_unexpanded_node() && num_ios < io_limit)
    {
        ...
        while (retset.has_unexpanded_node() && frontier.size() < beam_width && num_seen < beam_width)
        {
            ...
            if (iter != _nhood_cache.end())
            {
                ...
                if (_workload_logger.is_enabled()){
                    _workload_logger.record_cache_hit(_wl_query_id, nbr.id);
                }
            ...
            // ── VectorTailCache: record total visit count for PCS ─────────
            if (_collect_pcs_stats && nbr.id < _num_points)
            {
                _pcs_stats[nbr.id].total_visits.fetch_add(1, std::memory_order_relaxed);
            }
        }
...
#endif
            if (_workload_logger.is_enabled() && !frontier.empty()) {
                _workload_logger.record_miss_batch(_wl_query_id, frontier[0],
                                                (float)io_timer.elapsed(),
                                                (uint32_t)frontier.size());
            }
            if (stats != nullptr)
            {
                stats->io_us += (float)io_timer.elapsed();
                // ── VectorTailCache: per-node latency log (comment out after use) ────────
                if (_log_per_node_latency && !frontier.empty())
                {
                    float per_node_us = (float)io_timer.elapsed() / (float)frontier.size();
                    std::lock_guard<std::mutex> lk(_per_node_lat_mutex);
                    for (auto nid : frontier)
                        if (nid < _num_points)
                            _per_node_lat_log.push_back({nid, per_node_us});
                }
                // ─────────────────────────────────────────────────────────────────────────
            }

            // ── VectorTailCache: record per-miss SSD latency for PCS ─────
            if (_collect_pcs_stats && !frontier.empty())
            {
                float per_node_io_us = (float)io_timer.elapsed() / (float)frontier.size();
                for (auto nid : frontier)
                {
                    if (nid < _num_points)
                    {
                        auto &pcs_s = _pcs_stats[nid];
                        pcs_s.miss_count.fetch_add(1, std::memory_order_relaxed);
                        float old_val = pcs_s.miss_latency_us.load(std::memory_order_relaxed);
                        float new_val;
                        do {
                            new_val = old_val + per_node_io_us;
                        } while (!pcs_s.miss_latency_us.compare_exchange_weak(
                                     old_val, new_val,
...
#endif
    if (_workload_logger.is_enabled())
        _workload_logger.end_query(_wl_query_id, (float)query_timer.elapsed());
    ...
    if (stats != nullptr)
    {
        ...
        // ── VectorTailCache: mark tail queries for PCS ────────────────────
        if (_collect_pcs_stats && _pcs_tail_threshold_us > 0.0f)
        {
            float this_query_us = (float)query_timer.elapsed();
            if (this_query_us >= _pcs_tail_threshold_us)
            {
                for (auto vid : visited)
                {
                    if (vid < _num_points)
                    {
                        _pcs_stats[vid].tail_visits.fetch_add(
    ...
    // ── VectorTailCache: dump log when full (comment out after use) ──────────
    if (_log_per_node_latency && _per_node_lat_log.size() >= 800000)
    {
        _log_per_node_latency = false;
        std::ofstream f("per_node_latency.tsv");
        f << "node_id\tread_us\n";
        for (auto &e : _per_node_lat_log)
            f << e.first << "\t" << e.second << "\n";
        f.close();
        diskann::cout << "[PNL] Dumped " << _per_node_lat_log.size()
                    << " records to per_node_latency.tsv" << std::endl;
    }
    // ─────────────────────────────────────────────────────────────────────────
}
...
// ── VectorTailCache: generate_pcs_cache_list ─────────────────────────────────
template <typename T, typename LabelT>
void PQFlashIndex<T, LabelT>::generate_pcs_cache_list(std::string query_bin,
                                                       uint64_t l_search,
                                                       uint64_t beamwidth,
                                                       uint64_t num_nodes_to_cache,
                                                       uint32_t nthreads,
                                                       std::vector<uint32_t> &node_list,
                                                       float tail_percentile)
{
    diskann::cout << "[PCS] Starting Path Criticality Score cache generation." << std::endl;
    diskann::cout << "[PCS] tail_percentile=" << tail_percentile
    ...
    if (!file_exists(query_bin))
    {
        diskann::cerr << "[PCS] Query bin not found: " << query_bin
                      << ". Falling back to BFS cache." << std::endl;
        cache_bfs_levels(num_nodes_to_cache, node_list);
        return;
    }

    diskann::Timer pcs_total_timer;   // <-- ADDED

    // Phase 1: warm-up pass to measure latency distribution
    uint64_t sample_num, sample_dim, sample_aligned_dim;
    T *samples = nullptr;
    diskann::load_aligned_bin<T>(query_bin, samples, sample_num, sample_dim, sample_aligned_dim);
    diskann::cout << "[PCS] Loaded " << sample_num << " sample queries." << std::endl;

    std::vector<uint64_t> tmp_ids(sample_num, 0);
    std::vector<float>    tmp_dists(sample_num, 0);
    std::vector<float>    query_latencies(sample_num, 0.0f);

    diskann::cout << "[PCS] Phase 1: measuring latency distribution..." << std::endl;
    diskann::Timer phase1_timer;      // <-- ADDED
    auto warmup_stats = new diskann::QueryStats[sample_num];

#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
    for (int64_t i = 0; i < (int64_t)sample_num; i++)
    {
        cached_beam_search(samples + (i * sample_aligned_dim), 1, l_search,
                           tmp_ids.data() + i, tmp_dists.data() + i,
                           beamwidth, false, warmup_stats + i);
        query_latencies[i] = warmup_stats[i].total_us;
    }
    delete[] warmup_stats;
    diskann::cout << "[PCS][TIMING] Phase 1 took " << (phase1_timer.elapsed() / 1e6) << " s "
                  << "(" << sample_num << " queries, " << nthreads << " threads)" << std::endl;   // <-- ADDED


    // Set tail threshold at tail_percentile
    std::vector<float> sorted_lat = query_latencies;
    std::sort(sorted_lat.begin(), sorted_lat.end());
    size_t tail_idx = std::min((size_t)(tail_percentile * sorted_lat.size()),
                               sorted_lat.size() - 1);
    _pcs_tail_threshold_us = sorted_lat[tail_idx];

    diskann::cout << "[PCS] P50="   << sorted_lat[sorted_lat.size()/2] << "us"
                  << "  P90="       << sorted_lat[(size_t)(0.9f*sorted_lat.size())] << "us"
                  << "  threshold(" << (tail_percentile*100) << "%)="
                  << _pcs_tail_threshold_us << "us" << std::endl;

    // Phase 2: profiling pass — collect per-node PCS stats
    _pcs_stats.resize(_num_points);
    for (auto &s : _pcs_stats) {
        s.total_visits.store(0);
        s.tail_visits.store(0);
        s.miss_latency_us.store(0.0f);
        s.miss_count.store(0);
    }
    _collect_pcs_stats = true;

    diskann::cout << "[PCS] Phase 2: collecting per-node path criticality stats..." << std::endl;
    diskann::Timer phase2_timer;      // <-- ADDED
    auto pcs_stats_arr = new diskann::QueryStats[sample_num];
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
    for (int64_t i = 0; i < (int64_t)sample_num; i++)
    {
        cached_beam_search(samples + (i * sample_aligned_dim), 1, l_search,
                           tmp_ids.data() + i, tmp_dists.data() + i,
                           beamwidth, false, pcs_stats_arr + i);
    }
    _collect_pcs_stats = false;
    delete[] pcs_stats_arr;
    diskann::cout << "[PCS][TIMING] Phase 2 took " << (phase2_timer.elapsed() / 1e6) << " s "
                  << "(" << sample_num << " queries, " << nthreads << " threads)" << std::endl;   // <-- ADDED


    // Phase 3: compute PCS scores and select top-N nodes
    // PCS(v) = (tail_visits / total_visits) * avg_miss_latency
    diskann::cout << "[PCS] Phase 3: computing PCS scores..." << std::endl;
    diskann::Timer phase3_timer;      // <-- ADDED
    std::vector<std::pair<float, uint32_t>> pcs_scores;
    pcs_scores.reserve(_num_points);

    uint32_t visited_nodes = 0, tail_touched = 0, miss_nodes = 0;
    // Phase 3 — find sample_num for normalization
uint64_t sample_num_local = sample_num; // already available in scope

for (uint32_t i = 0; i < (uint32_t)_num_points; i++)
{
    uint32_t tv = _pcs_stats[i].total_visits.load();
    uint32_t mc = _pcs_stats[i].miss_count.load();
    float    ml = _pcs_stats[i].miss_latency_us.load();

    if (tv == 0) continue;
    visited_nodes++;
    if (mc > 0) miss_nodes++;

    float visit_freq   = (float)tv / (float)sample_num;
    float avg_miss_lat = mc > 0 ? (ml / (float)mc) : 0.0f;
    float pcs          = visit_freq * avg_miss_lat;
    pcs_scores.push_back({pcs, i});
}

    diskann::cout << "[PCS] Visited=" << visited_nodes
                  << "  Tail-touched=" << tail_touched
                  << "  Miss-nodes=" << miss_nodes << std::endl;
    ...
    node_list.clear();
    num_nodes_to_cache = std::min(num_nodes_to_cache, (uint64_t)pcs_scores.size());
    node_list.reserve(num_nodes_to_cache);
    for (uint64_t i = 0; i < num_nodes_to_cache; i++)
        node_list.push_back(pcs_scores[i].second);
    
    // // ── DUMP: PCS cache list + neighbor connections ───────────────────
    // {
    //     std::set<uint32_t> cached_set(node_list.begin(), node_list.end());
    //     // Print cached nodes
    //     for (auto node : node_list)
    //         diskann::cout << "[PCS_NODE] " << node << "\n";
    //     // Read neighbors of all cached nodes
    //     std::vector<T *> coord_bufs(node_list.size(), nullptr);
    //     std::vector<std::pair<uint32_t,uint32_t*>> nbr_bufs;
    //     for (size_t i = 0; i < node_list.size(); i++)
    ...
    // }
    // // ─────────────────────────────────────────────────────────────────
    diskann::cout << "[PCS][TIMING] Phase 3 took " << (phase3_timer.elapsed() / 1e6) << " s" << std::endl;   // <-- ADDED

    if (!pcs_scores.empty())
    {
        diskann::cout << "[PCS] Top-3 PCS scores: "
                      << pcs_scores[0].first << ", "
                      << (pcs_scores.size()>1 ? pcs_scores[1].first : 0.f) << ", "
                      << (pcs_scores.size()>2 ? pcs_scores[2].first : 0.f) << std::endl;
        diskann::cout << "[PCS] Selected " << node_list.size()
                      << " nodes. Min PCS in selection: "
                      << pcs_scores[num_nodes_to_cache-1].first << std::endl;
    }

    _pcs_stats.clear();
    _pcs_stats.shrink_to_fit();
    _pcs_tail_threshold_us = 0.0f;
    diskann::aligned_free(samples);
    diskann::cout << "[PCS] Cache list generation complete." << std::endl;
}
// ─────────────────────────────────────────────────────────────────────────────
