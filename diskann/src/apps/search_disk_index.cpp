...
// Modified by VectorTailCache:
//   --cache_policy  [bfs | frequency | pcs]
//   --log_workload  <path.csv>   per-query workload logger
...
int search_disk_index(diskann::Metric &metric,
                      ...
                      const float pcs_tail_percentile,
                      const std::string &workload_log,
                      const bool use_reorder_data = false)
{
    ...
    diskann::cout << "Cache policy: " << cache_policy;
    if (cache_policy == "pcs")
        diskann::cout << "  (tail_percentile=" << pcs_tail_percentile << ")";
    diskann::cout << std::endl;

    if (!workload_log.empty())
        diskann::cout << "Workload log: " << workload_log << std::endl;
    ...
    // ── VectorTailCache: enable workload logger ───────────────────────────────
    if (!workload_log.empty())
        _pFlashIndex->enable_workload_logging(query_num);

    // ── VectorTailCache: cache policy selection ───────────────────────────────
    std::vector<uint32_t> node_list;
    diskann::cout << "Caching " << num_nodes_to_cache << " nodes around medoid(s)" << std::endl;

    if (cache_policy == "pcs")
    {
        uint32_t search_L = Lvec.empty() ? 80 : Lvec[Lvec.size() / 2];
        diskann::cout << "[PCS] Profiling with L=" << search_L
                      << ", beamwidth=" << beamwidth << std::endl;
        _pFlashIndex->generate_pcs_cache_list(
            query_file, search_L, beamwidth,
            num_nodes_to_cache, num_threads, node_list,
            pcs_tail_percentile);
    }
    else if (cache_policy == "frequency")
    {
        if (file_exists(warmup_query_file))
        {
            uint32_t search_L = Lvec.empty() ? 80 : Lvec[Lvec.size() / 2];
            _pFlashIndex->generate_cache_list_from_sample_queries(
                warmup_query_file, search_L, beamwidth,
                num_nodes_to_cache, num_threads, node_list);
        }
        else
        {
            diskann::cout << "Sample data not found. Falling back to BFS." << std::endl;
            _pFlashIndex->cache_bfs_levels(num_nodes_to_cache, node_list);
        }
    }else if (cache_policy == "bfs_matched")
    {
        // Runs the same profiling I/O as PCS (for matched exposure), but
        // discards the PCS-selected list and caches BFS's medoid-based
        // list instead. Isolates whether the profiling pass itself (not
        // node selection) drives the PCS-vs-BFS gap.
        uint32_t search_L = Lvec.empty() ? 80 : Lvec[Lvec.size() / 2];
        diskann::cout << "[BFS-MATCHED] Running PCS profiling pass (L=" << search_L
                      << ", beamwidth=" << beamwidth << ") — result discarded" << std::endl;
        std::vector<uint32_t> discarded_list;
        _pFlashIndex->generate_pcs_cache_list(
            query_file, search_L, beamwidth,
            num_nodes_to_cache, num_threads, discarded_list,
            pcs_tail_percentile);
        discarded_list.clear();
        discarded_list.shrink_to_fit();

        diskann::cout << "[BFS-MATCHED] Loading BFS medoid-based cache list" << std::endl;
        _pFlashIndex->cache_bfs_levels(num_nodes_to_cache, node_list);
    }
    else if (cache_policy == "bfs_matched_freq")
    {
        // Runs the same profiling I/O as Frequency (1 pass, for matched
        // exposure), but discards the frequency-selected list and caches
        // BFS's medoid-based list instead. Isolates whether Frequency's
        // profiling pass alone (not its node selection) drives its win
        // over plain BFS.
        uint32_t search_L = Lvec.empty() ? 80 : Lvec[Lvec.size() / 2];
        std::vector<uint32_t> discarded_list;
        if (file_exists(warmup_query_file))
        {
            diskann::cout << "[BFS-MATCHED-FREQ] Running frequency profiling pass (L=" << search_L
                          << ", beamwidth=" << beamwidth << ") — result discarded" << std::endl;
            _pFlashIndex->generate_cache_list_from_sample_queries(
                warmup_query_file, search_L, beamwidth,
                num_nodes_to_cache, num_threads, discarded_list);
        ...
        {
            diskann::cout << "[BFS-MATCHED-FREQ] Sample data not found. Using query_file instead." << std::endl;
            _pFlashIndex->generate_cache_list_from_sample_queries(
                query_file, search_L, beamwidth,
                num_nodes_to_cache, num_threads, discarded_list);
        }
        ...
        diskann::cout << "[BFS-MATCHED-FREQ] Loading BFS medoid-based cache list" << std::endl;
        _pFlashIndex->cache_bfs_levels(num_nodes_to_cache, node_list);
    }
    else // "bfs" — DiskANN default
    {
        _pFlashIndex->cache_bfs_levels(num_nodes_to_cache, node_list);
    }
...
#pragma omp parallel for schedule(dynamic, 1)
        for (int64_t i = 0; i < (int64_t)warmup_num; i++)
            _pFlashIndex->cached_beam_search(
                ...
                warmup_result_ids_64.data() + i,
                warmup_result_dists.data() + i, 4);
    ...
    for (uint32_t test_id = 0; test_id < Lvec.size(); test_id++)
    {
        ...
        if (beamwidth <= 0)
        {
            ...
            optimized_beamwidth = optimize_beamwidth(
                _pFlashIndex, warmup, warmup_num, warmup_aligned_dim, L, optimized_beamwidth);
        }
        else
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        diskann::convert_types<uint64_t, uint32_t>(
            query_result_ids_64.data(), query_result_ids[test_id].data(), query_num, recall_at);

        auto mean_latency = diskann::get_mean_stats<float>(
            stats, query_num, [](const diskann::QueryStats &s) { return s.total_us; });
        auto latency_999 = diskann::get_percentile_stats<float>(
            stats, query_num, 0.999, [](const diskann::QueryStats &s) { return s.total_us; });
        auto mean_ios = diskann::get_mean_stats<uint32_t>(
            stats, query_num, [](const diskann::QueryStats &s) { return s.n_ios; });
        auto mean_cpuus = diskann::get_mean_stats<float>(
            stats, query_num, [](const diskann::QueryStats &s) { return s.cpu_us; });
    ...
    // ── VectorTailCache: save workload log ────────────────────────────────────
    if (!workload_log.empty())
        _pFlashIndex->save_workload_log(workload_log);
    ...
    for (auto L : Lvec)
    {
        ...
        std::string cur = result_output_prefix + "_" + std::to_string(L) + "_idx_uint32.bin";
        diskann::save_bin<uint32_t>(cur, query_result_ids[test_id].data(), query_num, recall_at);
        cur = result_output_prefix + "_" + std::to_string(L) + "_dists_float.bin";
        diskann::save_bin<float>(cur, query_result_dists[test_id++].data(), query_num, recall_at);
    }
...
int main(int argc, char **argv)
{
    ...
    // VectorTailCache params
    std::string cache_policy;
    float       pcs_tail_percentile;
    std::string workload_log;

    po::options_description desc{
        program_options_utils::make_program_description(
            "search_disk_index", "Searches on-disk DiskANN indexes")};
    try
    {
        ...
        optional_configs.add_options()
            ...
            ("beamwidth,W",
             po::value<uint32_t>(&W)->default_value(2),
            ...
            ("search_io_limit",
             ...
             "Max #IOs for search. Default: uint32::max()")
            ("num_threads,T",
            ...
            // ── VectorTailCache ───────────────────────────────────────────────
            ("cache_policy",
             po::value<std::string>(&cache_policy)->default_value("bfs"),
             "Cache admission policy:\n"
             "  bfs       = DiskANN default BFS from medoid\n"
             "  frequency = sample-query visit frequency\n"
             "  pcs       = VectorTailCache Path Criticality Score")
            ("pcs_tail_percentile",
             po::value<float>(&pcs_tail_percentile)->default_value(0.90f),
             "Tail percentile threshold for PCS profiling (default: 0.90)")
            ("log_workload",
             po::value<std::string>(&workload_log)->default_value(""),
             "Path to save per-query workload CSV (empty = disabled)");
            // ─────────────────────────────────────────────────────────────────
        ...
        if (cache_policy != "bfs" && cache_policy != "bfs_matched"  && cache_policy != "bfs_matched_freq" && cache_policy != "frequency" && cache_policy != "pcs")
        {
            std::cerr << "Invalid cache_policy: " << cache_policy
                      << ". Must be one of: bfs, frequency, pcs" << std::endl;
            return -1;
        }
    }
    ...
    {
        std::cout << "Error: Reorder data only supported for float." << std::endl;
        return -1;
    }
        ...
        if (!query_filters.empty() && label_type == "ushort")
        {
            if (data_type == std::string("float"))
                return search_disk_index<float, uint16_t>(
                    ...
                    fail_if_recall_below, query_filters,
                    cache_policy, pcs_tail_percentile, workload_log, use_reorder_data);
