#!/usr/bin/env python3
"""
gen_hdd_figures.py — Generate figures from HDD BFS vs PCS results
"""
...
# ── Args ──────────────────────────────────────────────────────────────────────
if len(sys.argv) < 2:
    print("Usage: python3 gen_hdd_figures.py <results_dir>")
    sys.exit(1)
...
# gen_sata_figures.py) bfs is always labeled above its point, pcs always
# below, regardless of which policy has the higher value at a given x.
...
for i, (ax, L) in enumerate(zip(axes, L_vals)):
    bfs_d = agg[
        (agg['policy'] == 'bfs') & (agg['L'] == L)
    ].set_index('cache_size')['p999_us']

    pcs_d = agg[
        (agg['policy'] == 'pcs') & (agg['L'] == L)
    ].set_index('cache_size')['p999_us']
...
finish_row(fig)
save(fig, 'hdd_fig3_mean')


# ── Summary table ─────────────────────────────────────────────────────────────
SEP = "─" * 72
print(f"\n{SEP}")
print("  HDD: BFS vs PCS SUMMARY")
print(SEP)
print(
