#!/usr/bin/env bash
# =============================================================================
# compare_beamwidth.sh — BFS vs PCS across multiple beamwidths
#
# Run order: ALL PCS runs first, then ALL BFS runs.
# This ensures no BFS warmup advantage from interleaved execution.
# =============================================================================
set -euo pipefail
...
OUT="${OUT_BASE}/beamwidth_${TIMESTAMP}"
RAW="${OUT}/raw"
mkdir -p "${RAW}"
...
# ── TSV header ────────────────────────────────────────────────────────────────
printf "policy\tbeamwidth\tcache_size\trun\tL\tqps\tmean_us\tp999_us\tmean_ios\tmean_io_us\trecall\ttail_amp\n" \
    > "${TSV}"

# ── Log parser ────────────────────────────────────────────────────────────────
parse_log() {
    local logfile="$1" policy="$2" bw="$3" cache="$4" run="$5"
    "${PYTHON}" - "$logfile" "$policy" "$bw" "$cache" "$run" "$TSV" << 'PYEOF'
import sys, re
logfile, policy, bw, cache, run, tsv = sys.argv[1:]
...
TOTAL_PCS=$(( ${#BEAMWIDTHS[@]} * RUNS ))
TOTAL_BFS=$(( ${#BEAMWIDTHS[@]} * RUNS ))

log "============================================================"
log "Beamwidth Comparison: BFS vs PCS"
...
# =============================================================================
log ""
log "─── PHASE 1: PCS (all beamwidths × all runs) ───────────────"
n=0
for bw in "${BEAMWIDTHS[@]}"; do
    for run in $(seq 1 $RUNS); do
        n=$(( n+1 ))
        logfile="${RAW}/pcs_bw${bw}_r${run}.log"
        log "[PCS ${n}/${TOTAL_PCS}] beamwidth=${bw} run=${run}"
        drop_cache
        "${SEARCH}" \
            ...
            --query_file "${QUERY}" \
            --gt_file "${GT}" \
            --result_path "${RAW}/res_pcs_bw${bw}_r${run}" \
            ...
            ${L_ARGS} \
            > "${logfile}" 2>&1
        parse_log "${logfile}" "pcs" "${bw}" "${CACHE_SIZE}" "${run}"
        log "  Done."
    done
done
...
# =============================================================================
log ""
log "─── PHASE 2: BFS (all beamwidths × all runs) ───────────────"
n=0
for bw in "${BEAMWIDTHS[@]}"; do
    for run in $(seq 1 $RUNS); do
        n=$(( n+1 ))
        logfile="${RAW}/bfs_bw${bw}_r${run}.log"
        log "[BFS ${n}/${TOTAL_BFS}] beamwidth=${bw} run=${run}"
        drop_cache
        "${SEARCH}" \
            --data_type float --dist_fn l2 \
            --index_path_prefix "${INDEX}" \
            --query_file "${QUERY}" \
            --gt_file "${GT}" \
            --result_path "${RAW}/res_bfs_bw${bw}_r${run}" \
            --num_nodes_to_cache "${CACHE_SIZE}" \
            --cache_policy bfs \
            -K 10 -T "${THREADS}" -W "${bw}" \
            ${L_ARGS} \
            > "${logfile}" 2>&1
        parse_log "${logfile}" "bfs" "${bw}" "${CACHE_SIZE}" "${run}"
        log "  Done."
    done
done
...
log "Generate figures:"
log "  python3 scripts/gen_beamwidth_figures.py ${OUT}"
log "============================================================"
